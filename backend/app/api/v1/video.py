...
#         # 1. Save file
#         filepath = await storage.save_upload(file_bytes,file.filename)
...
@router.post("/{video_id}/versions")
def create_versions(video_id: int, qualities: list[str], db: Session = Depends(get_db)):
    return video_service.generate_versions(db, video_id, qualities)  # sync


@router.get("/{video_id}/versions")
def list_versions(video_id: int, db: Session = Depends(get_db)):
    return video_service.get_versions(db, video_id)  # sync


@router.get("/{video_id}/versions/{quality}")
def download_version(video_id: int, quality: str, db: Session = Depends(get_db)):
    return video_service.get_version_file(db, video_id, quality)  # sync
