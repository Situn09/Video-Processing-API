...
#     job = await job_repo.find(job_id)
#     if not job:
...
#     vv = await db.get(VideoVersion, video_version_id)
#     if not vv:
#         raise HTTPException(status_code=404, detail="Video version not found")

#     return FileResponse(path=vv.filepath, filename=vv.filepath.split("/")[-1])
